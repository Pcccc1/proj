...
def sparse_feat_transform(df, item_raw_id2_idx_dict, feat_lbe_dict):
    df['hist_item_id'] = df['hist_item_id'].apply(lambda seq: [item_raw_id2_idx_dict[str(x)] for x in seq])

    for hist_id in var_len_feat:
        df[hist_id] = _pad_sequence(df[hist_id].tolist(), max_len=max_len, pad_value=0)
...
def organize_user_item_feat(df, item_feat_df, sparse_feat, dense_feat, item_content_vec_dict,
                            item_raw_id2_idx_dict, feat_lbe_dict, is_interest=True, is_w2v=False, word2vec_item_embed_dict=None):
    full_user_item_df = pd.merge(df, item_feat_df, on='item_id', how='left')
    full_user_item_df = fillna(full_user_item_df, sparse_feat, dense_feat)
    print('orgin data merge done!')

    if is_interest:
        full_user_item_df = obtain_user_hist_interest_feat(full_user_item_df, item_content_vec_dict)
        print('obtain user interest feat done!')
    
    if is_w2v:
        full_user_item_df = organize_word2vec_feat(full_user_item_df, word2vec_item_embed_dict)
        print('obtain word2vec feat done!')


    full_user_item_df = sparse_feat_transform(full_user_item_df, item_raw_id2_idx_dict, feat_lbe_dict)
    return full_user_item_df



def obtain_user_hist_interest_feat(full_user_item_df, item_content_vec_dict):
    ...
    def weighted_agg_content(hist_item_id_list):
        ...
        for loc, i in enumerate(hist_item_id_list):
            ...
            if i in item_content_vec_dict:
                weighted_content += loc_weight * item_content_vec_dict[i]
    ...
    img_interest_degree_array = target_item_vec[:, 128:256] * user_interest_vec[:, 128:256]
    ...
    full_user_item_df['interest_degree'] = full_user_item_df['txt_interest_degree'] + full_user_item_df['img_interest_degree']

    for f in ['interest_' + col for col in item_dense_feat] + ['img_interest_degree', 'txt_interest_degree', 'interest_degree']:
    ...
    def hist_2_target_cnt(hist_target_item_list, hist_no):
        ...
        if len(hist_target_item_list) >= hist_no:
            ...
            if hist_item in item_content_vec_dict:
                ...
                img_cnt_sim = np.dot(item_content_vec_dict[target_item][128:256], item_content_vec_dict[hist_item][128:256])
    ...
    def hist_2_target_time_diff(hist_time_list, hist_num=3):
        ...
        hist_time_diff = []
        for hist_time in hist_time_list[-hist_num:]:
...
def organize_word2vec_feat(full_user_item_df, w2v_item_embed_dict):
    def lookup_item_w2v_embed(item_id):
        return w2v_item_embed_dict.get(str(item_id), np.zeros(32)).tolist()
    ...
    item_w2v_embed_list = full_user_item_df['item_id'].apply(lookup_item_w2v_embed).tolist()
    item_w2v_cols = [f'item_w2v_embed_{i}' for i in range(32)]
